...
public interface DocumentSortableTag extends DocumentTag {
    ...
    void setSortNo(long sortNo);

    DocumentSortableKeyword getKeyword();

    void setKeyword(DocumentSortableKeyword keyword);

    // 순서번호도 중복이 되면 안된다.
    // 그래서 순서변경이 있을 때, 대규모 순서변경이 발생한다.
    // 그것들이 잘 처리되게 하는 역할은 postKeyword 엔티티가 전담한다.
    default void applySortNo(long newSortNo) {
        ...
        getKeyword().applySortNo(getDocumentSortableTag(), newSortNo);
    }

    DocumentSortableTag getDocumentSortableTag();
}
